...
format = 'public_mat';       % ECG format string
...
save_data = 1;             % Save .mat file of signals/annotations; 1 = yes, 0 = no
...
P = 1;      % Parallel iterating counter
 ...
 parfor (i = 1:num_files, workers)
	...
	try   % To deal with if ECG wont load due to some error in formatting
        ...
        disp(char(file_list{i}))
        ecg = ECG12(char(file_list{i}), format)
        all_data = load(char(file_list{i}));
        age = str2num(all_data.age);
        sex = all_data.sex;
        mi = all_data.mi;
		...
		fig_full_path = fullfile(fig_full_directory, basename);
		...
		% 	% close(sumfig);
        %     plot_and_save_ecg_leads(sig, fig_full_path)
		% end
		...
		if save_data
			...
			save_struct_parfor(data_directory, basename{1}, sig)
            plot_and_save_ecg_leads(sig, fig_full_path{1})
