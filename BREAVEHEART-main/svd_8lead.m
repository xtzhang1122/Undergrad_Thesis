...
function [s1, s2, s3, E3, timeRs, timeRe, var_s1_total, var_s2_total, var_s3_total, remaining_e] = svd_8lead(ecg, cutpt, fidpts, debug)
...
remaining_e = sqrt(newX(:,4).^2 + newX(:,5).^2 + newX(:,6).^2 + newX(:,7).^2 + newX(:,8).^2);
