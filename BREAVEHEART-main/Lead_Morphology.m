...
% Various measurements on the 12-lead ECG, given an ECG and a Beats annotation
    ...
    properties (SetAccess=public)
        ...
        L1_rs_ratio    % R/S ratio % different from original implementation
...
%         sai_vm_individual_iqr
    ...
    methods
        ...
        function obj = Lead_Morphology(varargin)        % varagin: ECG12, VCG, Beats, Annoparams
            ...
            num_12L_params = 18;     % Number of parameters for each of the 12L medians
            ...
            for j=1:length(lead_names_ecg)
                ...
                % newly added
                r_loc = find(signal == r_wave);
                
                % using Neural Network to annotate each lead
                % t_loc = temp
                nnet_sig = VCG(freq, ecg.units, signal);
                [q_loc, s_loc, ~, ~, ~, ~] = nnet_median_annotate(nnet_sig, 0);
                ...
                % Extract the range between S and T for J index
                if s_loc == -1
                    range = signal(r_loc + 20 : temp);
                else
                    range = signal(s_loc : temp);
                end
                
                derivative = diff(range);
                [~, zeroDerivativeIndex] = max(diff(derivative < 0));
            
                % Corresponding index in the original signal & Store the result in the list
                j_loc = s_loc + zeroDerivativeIndex - 1;

                % Ventricular Activation Time
                vat = round((1000/freq) * mean(signal(fidpts(1): r_loc)));

                % Calculate areas for 12L medians
                fidpts(end+1) = temp;
                fidpts(end+1) = j_loc;
                [qrs_area, t_area, jt_area] = mean_vector_leadmorph(signal, (1000/freq), fidpts, aps.baseline_flag);
                ...
                obj.(lead_morph_fields{step+4+(num_12L_params*(j-1))}) = rs_ratio;       % R/S ratio % different from original implementation
                ...
                % newly added
                obj.(lead_morph_fields{step+11+(num_12L_params*(j-1))}) = -1; % p
                obj.(lead_morph_fields{step+12+(num_12L_params*(j-1))}) = q_loc; % q
                obj.(lead_morph_fields{step+13+(num_12L_params*(j-1))}) = r_loc; % r
                obj.(lead_morph_fields{step+14+(num_12L_params*(j-1))}) = s_loc; % s
                obj.(lead_morph_fields{step+15+(num_12L_params*(j-1))}) = temp; % t max location (index)
                obj.(lead_morph_fields{step+16+(num_12L_params*(j-1))}) = j_loc; % j
                obj.(lead_morph_fields{step+17+(num_12L_params*(j-1))}) = jt_area; % jt_area
                obj.(lead_morph_fields{step+18+(num_12L_params*(j-1))}) = vat; % j
                
            end   % end for loop
            
            disp('exit first for loop')
            disp(lead_names_vcg)
...
%             obj.sai_vm_individual_iqr = iqr(sai_vm);
    ...
    methods(Static)
        ...
        function obj = adjustLocProperties(obj)
            % Define the leads to adjust
            leadsToAdjust = {'L1', 'L2', 'L3', 'avF', 'avL', 'avR', 'V1', 'V2', 'V3', 'V4', 'V5', 'V6'};

            % Get a list of all properties of the class
            ...
            for i = 1:length(propList)
                ...
                % Check if the property is one of the ECG leads and ends with '_loc'
                if any(startsWith(propName, leadsToAdjust)) && endsWith(propName, '_loc')
                    % Adjust the property value
                    originalValue = obj.(propName);
                    adjustedValue = round(originalValue * 0.9);
                    obj.(propName) = adjustedValue;
                end
            end
